...
const statusConfig = {
  PENDING: { label: "Chờ duyệt", bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-100" },
  APPROVED: { label: "Đã duyệt", bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-100" },
  REJECTED: { label: "Từ chối", bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-100" },
};

const ModerationBadge = ({ status }) => {
  const cfg = statusConfig[status] || statusConfig.PENDING;
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-extrabold border ${cfg.bg} ${cfg.text} ${cfg.border}`}>
      {cfg.label}
    </span>
  );
};
...
const AdminForumPosts = () => {
  ...
  useEffect(() => { loadData(); }, [filterStatus]); // eslint-disable-line
  ...
  useEffect(() => { setRejectionReason(""); }, [selectedPost?.id]);
  ...
  useEffect(() => { return () => { if (noticeTimer) clearTimeout(noticeTimer); }; }, [noticeTimer]);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return posts;
    return posts.filter(post =>
      [post.title, post.content, post.authorName, post.authorEmail, post.categoryName]
  ...
  const handleModerate = async (post, action) => {
    if (!post) return;
    if (action === "REJECTED" && !rejectionReason.trim()) { alert("Vui lòng nhập lý do từ chối"); return; }
    setProcessing(true);
    try {
      await verifyForumPost(post.id, action, rejectionReason.trim() || null);
      setPosts(current =>
        current.map(item => item.id === post.id ? { ...item, status: action, rejectionReason: action === "REJECTED" ? rejectionReason.trim() || null : null } : item)
          .filter(item => filterStatus === "ALL" || item.status === filterStatus)
      );
      setSelectedPost(null);
      setRejectionReason("");
      setNotice({ type: action === "APPROVED" ? "success" : "info", message: action === "APPROVED" ? `Đã duyệt bài "${post.title}".` : `Đã từ chối bài "${post.title}".` });
      await loadData();
    } catch (err) {
      alert(err?.response?.data?.message || err?.message || "Không thể xử lý bài viết");
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5 sm:space-y-6 pb-4">
      {notice && (
        <div className={`border text-sm rounded-xl px-4 py-3 ${notice.type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-sky-50 border-sky-200 text-sky-700"}`}>
          {notice.message}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <BookOpen size={22} weight="duotone" className="text-[#5A63F6]" /> Duyệt bài cộng đồng
          </h1>
          <p className="text-sm text-slate-400 font-medium mt-1">Duyệt các bài do người dùng đăng trước khi hiển thị công khai</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[
          { Icon: Warning, label: "Chờ duyệt", value: stats.pending, color: "text-amber-700", bg: "bg-amber-50", weight: "duotone" },
          { Icon: CheckCircle, label: "Đã duyệt", value: stats.approved, color: "text-emerald-700", bg: "bg-emerald-50", weight: "fill" },
          { Icon: XCircle, label: "Từ chối", value: stats.rejected, color: "text-rose-700", bg: "bg-rose-50", weight: "fill" },
        ].map(item => (
          <div key={item.label} className={`${item.bg} border border-slate-100 rounded-2xl p-5 flex items-center gap-4`}>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <p className="text-sm text-slate-500 font-medium">{filtered.length} bài viết</p>
          <div className="relative w-full sm:w-80">
            <MagnifyingGlass size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Tìm theo tiêu đề, tác giả, danh mục..." value={search}
        ...
        <div className="overflow-x-auto min-h-[300px]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center">
              <Warning size={32} weight="duotone" className="text-slate-300 mb-3" />
                ...
                {filtered.map(post => (
                  <tr key={post.id} className="hover:bg-slate-50/50 transition-colors group align-top">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">
                      ...
                      <div className="text-xs text-slate-400">{post.authorEmail}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">{post.categoryName}</td>
                    <td className="px-6 py-4"><ModerationBadge status={post.status} /></td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2 flex-wrap">
                        ...
                        {post.status === "PENDING" && (
                          <>
                            <button onClick={() => handleModerate(post, "APPROVED")}
          ...
          <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-extrabold text-slate-900">Duyệt bài cộng đồng</h2>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Tiêu đề</div>
                    ...
                    <div className="mt-3 text-sm text-slate-500 flex flex-wrap gap-3">
                      <span className="inline-flex items-center gap-1"><UserCircle size={14} weight="duotone" /> {selectedPost.authorName}</span>
                      <span className="inline-flex items-center gap-1"><Clock size={14} /> {selectedPost.createdAt ? new Date(selectedPost.createdAt).toLocaleString("vi-VN") : "—"}</span>
                    </div>
                  </div>
                  <div className="text-sm text-slate-500 bg-white border border-slate-200 rounded-xl px-3 py-2">{selectedPost.categoryName}</div>
                </div>
              </div>
              <div>
                <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Nội dung</p>
                <p className="text-sm text-slate-700 bg-slate-50 p-4 rounded-xl border border-slate-100 whitespace-pre-wrap">{selectedPost.content}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Tags</p>
                <div className="flex flex-wrap gap-2">
                  {(selectedPost.tags || []).length > 0
                    ? selectedPost.tags.map(tag => (
                        <span key={tag} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-violet-50 text-violet-700 text-xs font-bold border border-violet-100">
                          <Tag size={12} /> {tag}
                        </span>
                      ))
              ...
              {selectedPost.status === "REJECTED" && selectedPost.rejectionReason && (
                <div className="bg-rose-50 border border-rose-100 rounded-xl p-4">
                  <p className="text-xs font-bold text-rose-600 uppercase mb-1">Lý do từ chối</p>
                  <p className="text-sm text-rose-800">{selectedPost.rejectionReason}</p>
                </div>
              )}
              {selectedPost.status === "PENDING" && (
                <div>
                  <p className="text-xs font-bold text-slate-500 mb-1 uppercase tracking-wider">Nếu từ chối, nhập lý do</p>
            ...
            {selectedPost.status === "PENDING" && (
              <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50 shrink-0">
                <button onClick={() => handleModerate(selectedPost, "REJECTED")} disabled={processing}
                  className="px-5 py-2.5 rounded-xl text-red-600 font-bold hover:bg-red-50 text-sm transition-colors disabled:opacity-50">
                  Từ chối
                </button>
                <button onClick={() => handleModerate(selectedPost, "APPROVED")} disabled={processing}
                  ...
                  {processing ? <CircleNotch size={16} className="animate-spin" /> : <CheckCircle size={16} weight="fill" />}
                  Duyệt & hiển thị
                </button>
              </div>
            )}
