    ...
    return (
      <div className="text-center py-16">
        ...
        <h3 className="text-xl font-extrabold text-slate-800 mb-2">
          Chưa có kỹ năng nào được duyệt
        </h3>
        <p className="text-slate-500 text-sm max-w-sm mx-auto">
          Bạn cần đăng ký kỹ năng dạy và chờ Admin duyệt trước khi tạo lịch
          rảnh.
        </p>
        <a
          href="/app/teaching"
  ...
  return (
    <div>
      <h2 className="text-lg font-extrabold text-slate-900 mb-1">
        Chọn kỹ năng để tạo lịch dạy
      </h2>
      <p className="text-xs text-slate-400 mb-5">
        Chỉ hiển thị kỹ năng đã được Admin duyệt ✅
      </p>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`font-extrabold text-sm truncate ${selected ? "text-violet-700" : s.text}`}
                >
                  {skill.skillName}
                </p>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {skill.skillCategory} · {skill.level}
                </p>
                <p className="text-[11px] text-amber-600 font-bold mt-1 flex items-center gap-1">
                  <Lightning size={10} weight="fill" /> {skill.creditsPerHour}{" "}
                  credits/giờ
                </p>
              </div>
              {selected && (
      ...
      {data.teachingSkill && (
        <div
          className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 ${style.bg} ${style.border} mb-6`}
        >
          <div className="w-8 h-8 flex items-center justify-center">
            <SkillIcon
              iconName={data.teachingSkill.skillIcon}
              skillName={data.teachingSkill.skillName}
              category={data.teachingSkill.skillCategory}
              size={24}
              weight="duotone"
            />
          </div>
          <div>
            <p className={`font-extrabold text-sm ${style.text}`}>
              Đã chọn: {data.teachingSkill.skillName}
            </p>
            <p className="text-xs text-slate-500">
              {data.teachingSkill.outcomeDesc}
            </p>
          </div>
        </div>
      )}
...
const Step2 = ({ data, setData, onNext, onBack }) => {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-slate-400 mb-5">
        Chọn nhiều ngày + nhiều giờ → hệ thống tạo tất cả tổ hợp (dates x
        times). Slot trùng tự được bỏ qua.
      </p>

      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 mb-6">
        <span className="text-lg shrink-0">💡</span>
        <p className="text-xs text-amber-700">
          Học viên sẽ thấy các slot <strong>Trống</strong> và đặt lịch trực
          tiếp. Credits chuyển ngay khi đặt.
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 max-h-72 overflow-y-auto pr-1">
            {DAYS.map(({ iso, label }) => {
              ...
              return (
                ...
                >
                  {sel && (
                    <Check size={10} weight="bold" className="inline mr-1" />
                  )}
                  {label}
                </button>
              );
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {TIME_OPTIONS.map((t) => {
              ...
              return (
                ...
                >
                  {sel && (
                    <Check size={11} weight="bold" className="inline mr-1" />
                  )}
                  {t}
                </button>
              );
      ...
      {selectedDates.length > 0 && selectedTimes.length > 0 && (
        <p className="text-sm text-violet-700 font-bold bg-violet-50 px-4 py-2 rounded-xl inline-block mb-4">
          Sẽ tạo tối đa{" "}
          <strong>{selectedDates.length * selectedTimes.length}</strong> slot
      ...
      {selectedDates.length === 0 || selectedTimes.length === 0 ? (
        <p className="text-xs text-amber-600 font-semibold mb-4">
          ⚠️ Vui lòng chọn ít nhất 1 ngày và 1 giờ.
        </p>
      ) : null}
...
const Step3 = ({ data, onBack, onSubmit, submitting, submitError }) => {
  ...
  const slotCount =
    (data.selectedDates?.length ?? 0) * (data.selectedTimes?.length ?? 0);
  ...
  return (
    <div>
      ...
      {s && (
        <div
          className={`rounded-2xl border-2 p-5 mb-4 ${categoryStyle(s.skillCategory).bg} ${categoryStyle(s.skillCategory).border}`}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 flex items-center justify-center">
              <SkillIcon
                iconName={s.skillIcon}
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className={`font-extrabold text-base ${categoryStyle(s.skillCategory).text}`}
                >
                  {s.skillName}
                </span>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-4">
        <p className="text-xs font-bold text-slate-500 mb-3 flex items-center gap-1">
          ...
          />
          Slot sẽ được tạo: tối đa{" "}
          <strong className="text-violet-600 ml-1">{slotCount} slot</strong>
        </p>
        <div className="flex flex-wrap gap-2">
          {(data.selectedDates ?? []).slice(0, 4).map((d) => (
            <span
              key={d}
              className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100"
            >
              {d}
            </span>
          ))}
          {(data.selectedDates ?? []).length > 4 && (
            <span className="text-[11px] text-slate-400">
              +{data.selectedDates.length - 4} ngày nữa
            </span>
          )}
        </div>
        <div className="flex flex-wrap gap-2 mt-2">
          {(data.selectedTimes ?? []).map((t) => (
            <span
              key={t}
              className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100"
            >
              {t}
            </span>
          ))}
      ...
      <div className="bg-sky-50 border border-sky-200 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <p className="text-xs text-sky-600">
          Sau khi tạo, học viên sẽ thấy các slot <strong>Trống</strong> và có
          thể đặt lịch ngay.
        </p>
      </div>
...
const SuccessScreen = ({ data, createdCount, onDone }) => (
  <div className="text-center py-10">
    ...
    <p className="text-slate-500 text-sm mb-4">
      Đã tạo <strong className="text-violet-600">{createdCount} slot</strong>{" "}
      cho kỹ năng <strong>{data.teachingSkill?.skillName}</strong>.
    </p>
...
const CreateTeachingSession = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setApprovedSkills(
          all.filter((s) => s.verificationStatus === "APPROVED"),
        );
      } catch (_) {
        setApprovedSkills([]);
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError(
        e?.response?.data?.message || "Tạo slot thất bại. Vui lòng thử lại.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="max-w-3xl mx-auto font-sans pb-14">
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
          <Sparkle size={24} weight="duotone" className="text-amber-400" /> Tạo
          lịch dạy mới
        </h1>
        <p className="text-sm text-slate-400 mt-1">
          Chọn kỹ năng đã duyệt → chọn ngày/giờ → xác nhận
        </p>
      </div>
