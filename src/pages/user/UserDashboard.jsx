...
const ActionCard = ({ icon: Icon, bg, iconColor, title, sub, badge, to }) => (
  <Link
    to={to}
    className="relative bg-white/60 backdrop-blur-md rounded-2xl border border-white/40 p-5 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(90,99,246,0.15)] hover:-translate-y-1 transition-all duration-300 flex flex-col gap-3 group"
  >
    {badge && (
    ...
    <div
      className={`w-11 h-11 rounded-xl ${bg} flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300`}
    >
...
const SessionRow = ({ session, isTeacher }) => {
  ...
  const badgeText =
    session.status === "IN_PROGRESS" ? "Đang diễn ra" : "Sắp tới";
  const badgeCls =
  ...
  return (
    <div
      ...
      className="flex items-center gap-3 py-3 px-2 group cursor-pointer hover:bg-slate-50/50 rounded-xl transition-colors"
    >
      <div
        className={`w-10 h-10 rounded-[14px] ${isTeacher ? "bg-gradient-to-br from-violet-500 to-fuchsia-500" : "bg-gradient-to-br from-sky-400 to-indigo-500"} text-white flex items-center justify-center font-extrabold text-xs shrink-0 shadow-sm`}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-slate-500 font-medium mt-0.5">
          ...
          <span className="text-violet-600 font-bold">
            {session.slotTime ? session.slotTime.substring(0, 5) : "N/A"}
          </span>{" "}
          ({session.slotDate || "N/A"})
        </div>
      </div>
      <span
        className={`text-[10px] font-extrabold px-2.5 py-1 rounded-full shrink-0 ${badgeCls}`}
      >
        {badgeText}
      </span>
    </div>
...
const TeacherCard = ({ teacher }) => {
  ...
  return (
    <Link
      to={`/app/explore`}
      className="relative bg-white/80 backdrop-blur-sm rounded-2xl border border-violet-100/50 p-5 shadow-[0_4px_20px_-4px_rgba(90,99,246,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(90,99,246,0.2)] hover:border-violet-300 hover:-translate-y-1 transition-all duration-300 group"
    >
      ...
      <div className="flex items-center gap-3 mb-4 mt-1">
        <img
          ...
          alt="avatar"
          className="w-11 h-11 rounded-full border-2 border-white shadow-sm group-hover:scale-110 transition-transform duration-300 object-cover"
        />
        <div className="min-w-0">
          <p className="font-extrabold text-slate-800 text-sm truncate group-hover:text-violet-600 transition-colors">
            {teacher.userName || "Mentor"}
          </p>
          <p className="text-[11px] font-bold text-slate-400 truncate bg-slate-50 inline-block px-1.5 py-0.5 rounded mt-0.5">
            {teacher.skill?.name || "Kỹ năng"}
          </p>
        </div>
      </div>
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        setUpcomingTeaching(
          activeSessions.filter((s) => s.teacherId === user?.id).slice(0, 3),
        );
        setUpcomingLearning(
          activeSessions.filter((s) => s.learnerId === user?.id).slice(0, 3),
        );

        // Fetch Recommended Mentors (just grab approved teaching skills)
        const mentorsRes = await getApprovedTeachingSkills({
          page: 0,
          size: 3,
        });
        const mentorsObj = mentorsRes.data ?? mentorsRes;
        const mentorsList = mentorsObj.content
          ? mentorsObj.content
          : Array.isArray(mentorsObj)
            ? mentorsObj
            : [];
  ...
  const quickActions = [
    {
      ...
      sub: "Học và dạy",
      badge:
        upcomingTeaching.length + upcomingLearning.length > 0
          ? upcomingTeaching.length + upcomingLearning.length
          : null,
      to: "/app/sessions",
    },
    {
        ...
        <div className="bg-white/50 backdrop-blur-xl rounded-[2rem] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 sm:p-7 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {upcomingTeaching.length > 0 && (
              <Link
                to="/app/sessions"
                className="flex items-center gap-1 text-[11px] font-extrabold uppercase tracking-wide text-violet-600 hover:text-violet-700 hover:bg-violet-50 px-2 py-1 rounded-md transition-colors"
              >
          ...
          <div className="space-y-1 relative z-10">
            {loadingData ? (
              ...
              upcomingTeaching.map((s, i) => (
                <SessionRow key={i} session={s} isTeacher={true} />
              ))
            ) : (
              <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                ...
                <p className="text-xs text-slate-400 mb-4 mt-1 text-center max-w-[200px]">
                  Chưa có lịch dạy nào đang chờ đón. Hãy mở thêm slot dạy nhé!
                </p>
                <Link
                  to="/app/teaching/create"
                  className="text-xs font-extrabold text-violet-600 bg-violet-50 px-4 py-2 rounded-xl hover:bg-violet-100 transition-colors"
                >
                  Mở slot ngay
                </Link>
              </div>
        ...
        <div className="bg-white/50 backdrop-blur-xl rounded-[2rem] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 sm:p-7 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {upcomingLearning.length > 0 && (
              <Link
                to="/app/sessions"
                className="flex items-center gap-1 text-[11px] font-extrabold uppercase tracking-wide text-sky-600 hover:text-sky-700 hover:bg-sky-50 px-2 py-1 rounded-md transition-colors"
              >
          ...
          <div className="space-y-1 relative z-10">
            {loadingData ? (
              <div className="flex justify-center py-6">
                <CircleNotch size={24} className="animate-spin text-sky-400" />
              </div>
            ) : upcomingLearning.length > 0 ? (
              upcomingLearning.map((s, i) => (
                <SessionRow key={i} session={s} isTeacher={false} />
              ))
            ) : (
              <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-3">
                ...
                <p className="text-sm font-bold text-slate-500">
                  Chuẩn bị thêm kiến thức?
                </p>
                <p className="text-xs text-slate-400 mb-4 mt-1 text-center max-w-[200px]">
                  Nâng cấp bản thân ngay bằng cách tìm Mentor xịn.
                </p>
                <Link
                  to="/app/explore"
                  className="text-xs font-extrabold text-sky-600 bg-sky-50 px-4 py-2 rounded-xl hover:bg-sky-100 transition-colors"
                >
                  Tìm khoá học
                </Link>
              </div>
