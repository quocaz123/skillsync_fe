      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8 animate-in slide-in-from-bottom-4 duration-500 fade-in">
          <div className="bg-white rounded-3xl border border-slate-200/60 p-8 shadow-sm relative overflow-hidden">
            <div
              className={`absolute top-0 right-0 w-64 h-64 rounded-full mix-blend-multiply filter blur-[80px] opacity-40 -z-0 pointer-events-none ${activeTab === "learner" ? "bg-blue-100" : "bg-purple-100"}`}
            ></div>
              ...
              <div>
                <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                  {activeTab === "learner" ? (
                    ...
                    <>
                      <Lightbulb className="text-purple-500" /> Kỹ năng giảng
                      dạy
                    </>
                  )}
                </h2>
                <p className="text-slate-500 mt-1">
                  {activeTab === "learner"
                    ? "Những chủ đề bạn muốn thành thạo"
                    : "Cập nhật chứng chỉ để minh chứng năng lực"}
                </p>
              </div>
              ...
              {activeTab === "learner" ? (
                learningInterests.length === 0 ? (
                  <p className="text-slate-400 text-sm">
                    Bạn chưa đăng ký học kỹ năng nào.
                  </p>
                ) : (
                ...
                <p className="text-slate-400 text-sm">
                  Bạn chưa có kỹ năng dạy nào. Thêm ngay!
                </p>
              ) : (
                teachingSkills.map((ts) => (
                  <div
                    key={ts.id}
                    className="group flex items-center gap-2 px-4 py-2.5 rounded-xl border font-semibold text-sm bg-purple-50/50 border-purple-200 text-purple-700 hover:shadow-purple-200 hover:-translate-y-1 transition-all shadow-sm"
                  >
                    <SkillIcon
                      iconName={ts.skillIcon}
                      skillName={ts.skillName}
                      category={ts.skillCategory}
                      size={16}
                      weight="duotone"
                    />
                    <span>{ts.skillName}</span>
                    <span className="text-[10px] bg-purple-100 px-1.5 py-0.5 rounded font-bold">
                      {ts.level}
                    </span>
                    <button
          ...
          <div className="bg-white rounded-3xl border border-slate-200/60 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <UserRound className="text-indigo-500" size={20} /> Giới thiệu
                bản thân
              </h3>
              {!bioEditing && (
                <button
              ...
              <p className="text-slate-600 leading-relaxed text-sm">
                {profile?.bio || (
                  <span className="italic text-slate-400">
                    Chưa có giới thiệu. Nhấn ✏️ để thêm.
                  </span>
                )}
              </p>
            )}
          ...
          <div className="bg-white rounded-3xl border border-slate-200/60 p-6 shadow-sm relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-xs text-slate-400 mt-2">
                {trustScore >= 80
                  ? "🌟 Uy tín cao — được cộng đồng tin tưởng"
                  : trustScore >= 50
                    ? "✅ Hoạt động ổn định"
