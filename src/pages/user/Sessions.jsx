      ...
      <div className="bg-gradient-to-br from-indigo-900 via-violet-900 to-fuchsia-900 rounded-[1.5rem] sm:rounded-[2.5rem] px-5 py-7 sm:p-12 relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-5 shadow-xl shadow-indigo-900/20">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md text-indigo-100 text-xs font-bold uppercase tracking-wider mb-4 border border-white/20">
            <CalendarDays size={14} className="text-fuchsia-300" /> Quản lý học
            tập
          </div>
          <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight mb-3">
            Lịch học của bạn
          </h1>
          <p className="text-indigo-200/80 font-medium text-lg max-w-md">
            Theo dõi lịch hẹn 1-kèm-1, tham gia video call và quản lý tiến trình
            của bạn.
          </p>
        </div>
        ...
        <div className="relative z-10 flex p-1.5 bg-white/10 backdrop-blur-md rounded-2xl w-full md:w-auto border border-white/20 shadow-inner">
          <button
            ...
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-bold transition-all duration-300 ${activeTab === "upcoming" ? "bg-white text-indigo-700 shadow-md scale-[1.02]" : "text-white hover:bg-white/10"}`}
          >
            Sắp diễn ra{" "}
            <span
              className={`text-[11px] px-2.5 py-0.5 rounded-full font-black ${activeTab === "upcoming" ? "bg-indigo-100 text-indigo-700" : "bg-white/20 text-white"}`}
            >
              ...
              {upcomingSessions.length === 0 ? (
                <div className="bg-white rounded-[2rem] border-2 border-dashed border-slate-200 p-16 text-center">
                  ...
                  <p className="text-slate-500 mb-8 max-w-sm mx-auto">
                    Vào mục Khám phá để tìm Mentor phù hợp và đặt lịch học ngay!
                  </p>
                  <a
                    href="/app/explore"
                    className="inline-flex items-center gap-2 px-8 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 hover:shadow-lg hover:-translate-y-1 transition-all"
                  >
                ...
                upcomingSessions.map((session, index) => (
                  <div
                    key={session.id}
                    className={`bg-white rounded-[2rem] border overflow-hidden transition-all hover:shadow-xl hover:shadow-indigo-900/5 ${index === 0 ? "border-indigo-500 ring-4 ring-indigo-50 shadow-md" : "border-slate-200"}`}
                  >
                    {index === 0 && (
                      <div className="bg-indigo-600 px-6 py-2.5 flex items-center gap-2.5">
                        ...
                        <span className="text-white text-xs font-black uppercase tracking-widest opacity-90">
                    ...
                    <div className="p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start md:items-center">
                      ...
                      <div className="flex-1 min-w-0 md:pl-2">
                        <h3 className="text-2xl font-black text-slate-900 mb-1 truncate">
                          {session.skillName}
                        </h3>
                        <p className="text-slate-500 font-medium mb-4 flex items-center gap-2">
                          <span>
                            {isTeacher(session) ? "Học viên:" : "Giảng viên:"}
                          </span>
                      ...
                      <div className="flex flex-col gap-3 shrink-0 w-full md:w-48">
                        {(() => {
                          ...
                          if (js.state === "too-early") {
                            ...
                            return (
                              <div className="w-full flex flex-col items-center justify-center gap-1 px-5 py-3.5 bg-amber-50 border-2 border-amber-200 text-amber-700 font-bold rounded-xl text-sm cursor-not-allowed select-none">
                                ...
                                <span className="text-[11px] font-semibold text-amber-500 opacity-80">
                                  Vào phòng sớm nhất 15 phút trước
                                </span>
                              </div>
                            );
                          }
                          ...
                          return (
                            ...
                            >
                              <Play size={18} fill="currentColor" /> Tham gia
                              học
                            </button>
                          );
                        })()}
              ...
              {pastSessions.length === 0 ? (
                <div className="lg:col-span-2 bg-white rounded-[2rem] border-2 border-dashed border-slate-200 p-16 text-center">
                  ...
                  <h3 className="text-2xl font-extrabold text-slate-700 mb-2">
                    Bộ sưu tập kỹ năng trống
                  </h3>
                  <p className="text-slate-400 max-w-sm mx-auto">
                    Chưa có buổi học nào được hoàn thành. Bắt đầu ngay để tích
                    lũy kiến thức tại đây nhé.
                  </p>
                </div>
              ) : (
                pastSessions.map((session) => (
                  <div
                    key={session.id}
                    className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all hover:-translate-y-1 flex flex-col"
                  >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-slate-500 font-semibold text-sm">
                            {isTeacher(session)
                              ? `Học viên: ${session.learnerName}`
                              : `Với ${session.teacherName}`}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between gap-6">
                      ...
                      {session.review ? (
                        <div className="bg-amber-50/50 rounded-2xl p-5 border border-amber-100/50 relative mt-auto">
                          ...
                          <p className="mt-2 text-sm text-slate-700 italic font-medium">
                            "{session.review}"
                          </p>
                        </div>
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-lg shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-8 duration-300 border border-white/20 overflow-hidden flex flex-col">
            <div className="bg-indigo-600 p-8 text-center relative">
              <button
                ...
                className="absolute top-6 right-6 w-8 h-8 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center text-white transition-colors"
              >
                <XCircle size={20} />
              </button>
              ...
              <h2 className="text-2xl font-black text-white px-4 leading-tight">
                {reviewModal.skillName}
              </h2>
              <p className="text-indigo-200 mt-2 font-medium">
                Mentor: {reviewModal.teacherName}
              </p>
            </div>
            <div className="p-8">
              <div className="mb-8 text-center">
                <p className="text-sm font-black text-slate-400 mb-4 uppercase tracking-widest">
                  Trải nghiệm của bạn?
                </p>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-lg shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-8 duration-300 border border-slate-100 overflow-hidden flex flex-col">
            <div className="bg-red-50 p-6 border-b border-red-100 relative flex items-center gap-4">
              <button
                ...
                className="absolute top-6 right-6 w-8 h-8 bg-white hover:bg-red-100 rounded-full flex items-center justify-center text-red-500 transition-colors shadow-sm"
              >
                <XCircle size={20} />
              </button>
              ...
              <div>
                ...
                <p className="text-slate-500 font-medium text-sm">
                  Với Mentor {reportModal.teacherName}
                </p>
              </div>
            </div>
            <div className="p-8">
              <div className="mb-6">
                ...
                >
                  <option value="TEACHER_LATE">
                    Mentor vào trễ / Không tham gia
                  </option>
                  <option value="POOR_QUALITY">
                    Chất lượng giảng dạy kém / Không đúng mong đợi
                  </option>
                  <option value="TECHNICAL_ISSUE">
                    Sự cố kỹ thuật (Mạng, Mic, App...)
                  </option>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[3rem] w-full max-w-sm p-10 shadow-2xl text-center flex flex-col items-center relative overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <p className="text-slate-500 mb-8 font-medium">
              Cảm ơn bạn đã đánh giá buổi học{" "}
              <span className="text-slate-700 font-bold">
                {successModal.skillName}
              </span>
              .
            </p>
            <button
              onClick={() => setSuccessModal(null)}
              className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl transition-all shadow-lg shadow-emerald-500/30 hover:-translate-y-1 active:translate-y-0 text-lg"
            >
              Đóng lại
            </button>
          </div>
