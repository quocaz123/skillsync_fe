    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full border border-slate-200 p-6">
        ...
        <p className="text-sm text-slate-600 mb-6">
          Giao diện làm quiz sẽ kết nối API khi backend sẵn sàng. Hiện tại đây
          là bản demo.
        </p>
        <button
          type="button"
...
export default function LearningPathStudy() {
    ...
    fetchUserLearningPath(pathId)
      .then((d) => {
        if (!c) {
          ...
          setSelectedModuleId(
            d.currentModuleId || ongoingId || firstId || null,
          );
        }
      })
      .catch((e) => {
  ...
  const handlePrimaryCta = () => {
    ...
    if (!pc) return;
    if (
      (pc.type === "CONTINUE_LESSON" || pc.type === "REVIEW_PATH") &&
      pc.lessonId
    ) {
      ...
      document
        .getElementById(`module-${pc.moduleId}`)
        ...
        <div className="p-6 sm:p-8">
                  ...
                  {data.currentModuleId && (
                    <span className="text-xs text-slate-500 font-semibold">
                      ...
                      <span className="text-slate-800">
                        {data.modules.find((m) => m.id === data.currentModuleId)
                          ?.title ?? "—"}
                      </span>
                    </span>
                  )}
          ...
          <div className="flex flex-wrap justify-between gap-2 mt-3 text-sm text-slate-600">
            <span className="font-semibold">
              Module: {data.completedModulesCount} / {data.totalModulesCount} đã
              xong
            </span>
            {data.lastInProgressLesson && (
              <span className="text-slate-500">
                ...
                <span className="text-slate-800 font-medium">
                  {data.lastInProgressLesson.title}
                </span>
              </span>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
            <Layers size={20} className="text-indigo-600" /> Chương trình theo
            module
          </h2>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-[11px] text-slate-500 font-semibold">
                        ...
                        {mod.quiz?.enabled && (
                          <span>
                            Quiz:{" "}
                            {QUIZ_BADGE[mod.quiz.status]?.text ??
                              mod.quiz.status}
                          </span>
                        )}
                        <span>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-500">
                          ...
                          {LESSON_STATUS_LABEL[les.status]}
                        </p>
                      </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      >
                        {QUIZ_BADGE[selectedModule.quiz.status]?.text ??
                          selectedModule.quiz.status}
                      </span>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      {selectedModule.quiz.status === "FAILED"
                        ? "Làm lại"
                        : "Làm quiz"}
                    </button>
                  </div>
              ...
              {selectedModule.mentorSupport && data.mentor && (
                <div className="rounded-2xl border border-violet-200 bg-violet-50/50 p-5">
                  ...
                  <p className="text-sm text-slate-600 mb-4">
                    Mentor sẽ hỗ trợ khi bạn gặp khó trong module này — gửi mô
                    tả cụ thể để được phản hồi nhanh hơn.
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedModule.supportRequest?.status === "SENT" && (
                      <>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            openSupportForModule(selectedModule.id)
                          }
                    ...
                    {selectedModule.supportRequest?.status ===
                      "IN_PROGRESS" && (
                      <button
                        type="button"
                        className="px-4 py-2 rounded-xl border border-violet-300 text-sm font-bold bg-white"
                      >
                        Xem tiến độ hỗ trợ
                      </button>
                    )}
