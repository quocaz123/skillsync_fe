...
const Login = () => {
    ...
    const handleEmailLogin = (e) => {
        ...
        // Mock Auth Logic: Phân quyền admin nếu nhập email hệ thống
        const role = email === 'admin@skillsync.com' ? 'admin' : 'user';
        const name = email.split('@')[0];

        login({ id: Date.now().toString(), name: name, email: email, role: role });
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-8 relative z-10">
                ...
                <p className="text-xs text-center text-slate-400 mt-4">
                    Hint: Use <span className="font-mono bg-slate-100 px-1 rounded">admin@skillsync.com</span> to login as Admin.
                </p>
            </div>
        </div>
    );
