...
const AdminLayout = () => {
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          path: "/admin/teaching-skills",
          label: "Xét duyệt Mentor",
          icon: ShieldCheck,
        },
    ...
    <>
      ...
      <div className="h-16 flex items-center px-4 border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-3 overflow-hidden ml-1">
          ...
          />
          {!isCollapsed && (
            <span className="text-xl font-bold bg-clip-text text-transparent bg-linear-to-r from-violet-600 to-violet-800 shrink-0 whitespace-nowrap animate-in fade-in duration-300">
              SkillSync Admin
            </span>
          )}
        </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    {!isCollapsed && (
                      <span className="shrink-0 whitespace-nowrap animate-in fade-in duration-200">
                        {item.label}
                      </span>
                    )}
                  </Link>
      ...
      <div
        className={`p-4 border-t border-slate-200 shrink-0 ${isCollapsed ? "items-center flex flex-col" : ""}`}
      >
        <div
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside
        className={`hidden md:flex ${isCollapsed ? "w-20" : "w-64"} bg-white border-r border-slate-200 flex-col transition-all duration-300 relative z-20 shrink-0`}
      >
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-72 bg-white border-r border-slate-200 flex flex-col z-40 md:hidden transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <button
      ...
      <main className="flex-1 flex flex-col overflow-y-auto w-full min-w-0 relative">
        ...
        <header className="h-14 md:h-16 bg-white border-b border-slate-200 flex items-center justify-between px-3 md:px-8 z-10 shrink-0 sticky top-0">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex items-center gap-2 sm:hidden">
              ...
              <span className="text-sm font-bold bg-clip-text text-transparent bg-linear-to-r from-violet-600 to-violet-800">
                Admin
              </span>
            </div>
          </div>
        ...
        <div
          ...
          className="flex-1 p-3 sm:p-5 md:p-8 bg-slate-50/50 w-full"
        >
          <Outlet />
        </div>
      </main>
