...
const EVIDENCE_DEFS = [
  ...
  },
  {
    ...
    acceptUrl: true,
    icon: LinkedinLogo,
    tips: "Link hồ sơ (Nên có Endorsements)",
  },
  {
    id: "work",
    label: "Dự án / Nơi làm việc",
    type: "optional",
    pts: 15,
    uploadType: "TEACHING_EVIDENCE",
    acceptUrl: true,
...
export const AddSkillModal = ({ onClose, onSave }) => {
  ...
  // Step 2 validation
  const step2Valid =
    experienceDesc.trim().length >= 20 && outcomeDesc.trim().length >= 10;

  const handleFileUpload = async (evDef, file) => {
    setEvidenceData((prev) => ({
      ...prev,
      [evDef.id]: { ...prev[evDef.id], uploading: true, done: false },
    }));
    try {
      ...
      setEvidenceData((prev) => ({
        ...prev,
        [evDef.id]: { ...prev[evDef.id], uploading: false, done: false },
      }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const evPromises = EVIDENCE_DEFS.filter(
        (ev) => evidenceData[ev.id]?.done,
      ).map((ev) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex justify-center items-center px-4 py-8 bg-slate-900/40 backdrop-blur-sm"
            ...
            <h2 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
              ...
              {step === 1
                ...
                : step === 2
                  ? "Chi tiết giảng dạy"
                  : step === 3
                    ? `Chứng minh năng lực — ${selectedSkill?.name}`
                    : "Xác nhận & lưu"}
            </h2>
            <p className="text-sm text-slate-500 mt-1">
              {step === 1
                ? "Chọn kỹ năng và cấp độ của bạn"
                : step === 2
                  ? "Mô tả kinh nghiệm và phong cách dạy của bạn"
                  : step === 3
                    ? "Upload bằng chứng phù hợp với nhóm kỹ năng"
                    : "Kiểm tra lại trước khi thêm vào hồ sơ"}
            </p>
          </div>
            ...
            <div className="hidden sm:flex items-center gap-2">
              {[1, 2, 3, 4].map((s, i) => (
                <div key={s} className="flex items-center">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${step >= s ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-400"}`}
                  >
                    {step > s ? <Check size={14} weight="bold" /> : s}
                  </div>
                  {i < 3 && (
                    <div
                      className={`w-8 h-0.5 mx-2 ${step > s ? "bg-indigo-600" : "bg-slate-100"}`}
                    />
                  )}
                </div>
              ))}
                ...
                {loadingSkills ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {skills.map((skill) => {
                      ...
                      const colorClass =
                        skillColorByCat[skill.category] ||
                        skillColorByCat.OTHER;
                      return (
                        <button
                          key={skill.id}
                        ...
                        >
                          ...
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl mb-2.5 shadow-sm border ${colorClass}`}
                          >
                            <SkillDynamicIcon
                              skillName={skill.name}
                              defaultIcon={skill.icon}
                              size={24}
                              weight="duotone"
                            />
                          </div>
                          <p
                            className={`font-bold text-sm mb-0.5 ${isSelected ? "text-indigo-900" : "text-slate-800"}`}
                          >
                            {skill.name}
                          </p>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-3xl">
                        <SkillDynamicIcon
                          ...
                          defaultIcon={selectedSkill.icon}
                          size={32}
                          weight="duotone"
                        />
                      </div>
                      <div>
                        <p className="font-extrabold text-slate-900 flex items-center gap-1 text-sm">
                          ...
                          — hệ thống sẽ hỏi bạn cung cấp:
                        </p>
                        <p className="text-xs text-slate-500">
                          Kỹ năng chuyên môn — xác minh bằng trình độ hoặc kinh
                          nghiệm thực tế
                        </p>
                      </div>
                    </div>
                    ...
                    {LEVELS.map((lvl) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-bold mb-2 ${lvl.color}`}
                          >
                            {lvl.label}
                          </span>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 flex items-start gap-3">
                ...
                <div>
                  <p className="font-extrabold text-indigo-900 text-sm">
                    {selectedSkill?.name} · {selectedLevel?.label}
                  </p>
                  <p className="text-xs text-indigo-600 mt-0.5">
                    Học viên sẽ thấy những thông tin này trước khi đặt lịch
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  >
                    {experienceDesc.trim().length >= 20
                      ? "✓ Đạt yêu cầu"
                      : `Còn thiếu ${20 - experienceDesc.trim().length} ký tự`}
                  </span>
                  <span className="text-xs text-slate-400">
                    {experienceDesc.length} ký tự
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mb-2">
                  Mỗi dòng = 1 mục tiêu, hiển thị dưới dạng danh sách tick ✓
                  (tối thiểu 10 ký tự)
                </p>
                <textarea
                  value={outcomeDesc}
                ...
                {outcomeDesc.trim().length >= 10 && (
                  <div className="mt-2 p-3 bg-violet-50 border border-violet-100 rounded-xl">
                    <p className="text-[11px] font-bold text-violet-600 mb-1.5">
                      Preview — Học viên sẽ thấy:
                    </p>
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mb-2">
                  Bạn thường dạy theo cách nào? Thực hành, lý thuyết, hay
                  hands-on project?
                </p>
                <textarea
                  value={teachingStyle}
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="bg-emerald-50 border border-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider">
                      thêm để tăng trust score
                    </span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {EVIDENCE_DEFS.filter((e) => e.type === "optional").map(
                      (ev) => (
                        <EvidenceItem
                          key={ev.id}
                          ev={ev}
            ...
            <div className="space-y-6">
              <div className="bg-slate-50 border border-slate-100 rounded-3xl p-6">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex items-center flex-wrap gap-2 text-lg">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-lg border text-xs font-bold ${selectedLevel?.color}`}
                    >
                      {selectedLevel?.label}
                    </span>
                  </div>
              ...
              <div className="bg-cyan-50 border border-cyan-100 rounded-2xl p-5">
                <p className="text-sm text-cyan-800 leading-relaxed">
                  ...
                  Kỹ năng xuất hiện trong hồ sơ. Bạn có thể đăng buổi dạy cụ thể
                  từ trang <b className="text-cyan-900">Dạy học</b>. Bằng chứng
                  luôn có thể chỉnh sửa sau.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-b-3xl shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {saving ? (
                <>
                  <Spinner size={16} className="animate-spin" /> Đang lưu...
                </>
              ) : step === 4 ? (
                <>
...
const EvidenceItem = ({
  ...
  return (
    <div
      className={`border rounded-xl transition-all ${isDone ? "border-indigo-400 bg-indigo-50/20" : "border-slate-200 bg-white hover:border-slate-300"}`}
    >
      <div
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm shrink-0 ${isDone ? "bg-indigo-500 text-white" : "bg-slate-50 text-slate-500 border border-slate-100"}`}
          >
            {isUploading ? (
        ...
        <div className="flex items-center gap-3">
          {isDone && (
            <span className="text-xs font-bold text-indigo-600 hidden sm:flex items-center gap-1">
              <CheckCircle weight="fill" size={14} /> Đã tải
            </span>
          )}
          <CaretDown
            size={16}
      ...
      {expanded && (
        <div
          className="px-5 pb-5 border-t border-slate-100 pt-5 space-y-4 animate-slide-in"
          style={{ animationDelay: "0.1s" }}
        >
          {/* File upload option */}
          {ev.uploadType && (
            <div>
              <p className="text-[11px] font-extrabold text-slate-500 mb-2 uppercase tracking-wider">
                📎 Tải file lên
              </p>
              <label
                className={`relative overflow-hidden flex flex-col items-center justify-center gap-3 px-6 py-6 rounded-2xl border-2 border-dashed cursor-pointer transition-all ${state?.file ? "bg-indigo-50 border-indigo-200 hover:border-indigo-300" : "bg-slate-50 border-slate-200 hover:border-indigo-400 hover:bg-slate-100/50"}`}
              >
                {isUploading && (
                ...
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm ${state?.file ? "bg-indigo-100" : "bg-white border border-slate-200"}`}
                >
                  {state?.file ? (
                    <FileText
                      size={24}
