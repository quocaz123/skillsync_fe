...
export default function UserSkillCard({
    ...
    // Always use div to prevent button-in-button HTML violation.
    // If clickable, use an absolutely positioned inert overlay so action buttons still receive clicks.
    return (
        <div
        ...
        >
            {/* Invisible click overlay — sits behind buttons so they still get events */}
            {onClick && (
                <button
                    type="button"
                    onClick={onClick}
                    aria-label={skill.skillName}
                    className="absolute inset-0 rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"
                    style={{ zIndex: 0 }}
                />
            )}

            {/* All content sits above the overlay */}
            <div className="relative flex items-center gap-2 flex-wrap w-full" style={{ zIndex: 1 }}>
            ...
            {(showDesc && skill.experienceDesc) && (
                <p className="relative text-xs opacity-80 leading-relaxed max-w-full text-slate-700 mt-1 line-clamp-2 w-full" style={{ zIndex: 1 }}>{skill.experienceDesc}</p>
            )}

            {skill.verificationStatus === 'REJECTED' && skill.rejectionReason && (
                <p className="relative text-[11px] text-red-600 bg-white/70 p-2.5 rounded-xl italic font-medium w-full border border-red-100/50" style={{ zIndex: 1 }}>Phản hồi từ Admin: {skill.rejectionReason}</p>
            )}

            <div className="relative flex flex-wrap items-center justify-between w-full mt-1 gap-2" style={{ zIndex: 1 }}>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* actionButtons sit at z-index 1, above the overlay at z-index 0 */}
                    {actionButtons}
                </div>
            </div>
